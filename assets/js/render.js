...
var Render = (function () {
  ...
  function inline(s) {
    ...
    /* Жирный разбираем первым и нежадно: внутри него бывает курсив —
       «**механизм изменения *K*ш**». Старый шаблон [^*]+ такое не ловил. */
    t = t.replace(/\*\*([\s\S]+?)\*\*/g, '<strong>$1</strong>');
    /* Курсив — без требования пробела слева: в формулах обозначения идут
       подряд через знаки — *S*ок/*S*р, *M*ш = *Y*р·*x*р − *Y*ск·*x*ск. */
    t = t.replace(/\*([^*\n]+)\*/g, '<em>$1</em>');
    t = t.replace(/->/g, '→');
    return t;
  }
